  ...
  return (
    <div className="mx-auto w-full max-w-lg">
      ...
      <div className="overflow-hidden rounded-3xl border border-border/60 bg-card print:rounded-none print:border-0">
        {/* Masthead */}
        <div className="flex items-center gap-3 border-b border-border/60 px-7 py-5">
          <Logo className="h-10" />
          <div className="min-w-0">
            <p className="font-[family-name:var(--font-display)] text-base font-semibold leading-tight">Adepa Pork Hub</p>
            <p className="text-[11px] text-muted-foreground">A Symas Group company</p>
          </div>
        ...
        <div className="flex items-center gap-3 px-7 pt-5">
          ...
          <p className="text-[10.5px] font-bold uppercase tracking-wider text-muted-foreground">Party of {partySize}</p>
          <div className="h-px flex-1 bg-border" />
        </div>
        <div className="flex flex-col gap-2.5 px-7 pb-2 pt-4">
          {guests.map((name, i) => (
            <div key={i} className={`flex items-baseline justify-between gap-3 pb-2.5 text-sm ${i < guests.length - 1 ? "border-b border-dashed border-border" : ""}`}>
              <span className="flex items-baseline gap-2 truncate">
                <span className="font-mono text-[9.5px] text-muted-foreground">{String(i + 1).padStart(2, "0")}</span>
        ...
        {/* Payment */}
        <div className="mx-7 my-5 flex items-center justify-between gap-4 rounded-2xl border border-border/60 bg-secondary/40 px-4.5 py-4">
          <div>
        ...
        <div className="flex flex-col gap-4 px-7 py-6">
          ...
          <p className="rounded-lg border border-border/60 bg-secondary/40 px-3.5 py-2.5 font-mono text-[12.5px] text-muted-foreground">
